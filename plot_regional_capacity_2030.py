"""
Single Plot: Regional Renewable Capacity Distribution (2030, ETS2)
...
# Get 2030 data for ETS2 scenario
row_2030 = renewable_cap_df[renewable_cap_df.iloc[:, 0] == 2030]

if not row_2030.empty:
    # Extract regional capacities (ETS2 columns)
    regional_cap_ets2 = [
        row_2030.iloc[0, 17],  # Northwest
        row_2030.iloc[0, 14],  # Northeast
        row_2030.iloc[0, 8],   # Centre
        row_2030.iloc[0, 20],  # South
        row_2030.iloc[0, 11]   # Islands
    ]
    ...
    # Save
    plt.savefig('results/Single_Plot_Regional_Capacity_2030.png',
                bbox_inches='tight', dpi=300)
    plt.savefig('results/Single_Plot_Regional_Capacity_2030.pdf',
                bbox_inches='tight')
    print("✓ Regional renewable capacity plot (2030) saved!")
    print("  - results/Single_Plot_Regional_Capacity_2030.png")
    print("  - results/Single_Plot_Regional_Capacity_2030.pdf")

    # Print summary
    print("\n" + "="*70)
    print("REGIONAL RENEWABLE CAPACITY DISTRIBUTION (2030, ETS2)")
    print("="*70)
    ...
    if not row_2040.empty:
        ...
        print("\nGrowth from 2030 to 2040:")
        print(f"{'Region':<15} {'2030 (GW)':>12} {'2040 (GW)':>12} {'Growth (%)':>12}")
        print("-"*70)
        for i, region in enumerate(regions):
            growth = ((regional_cap_2040[i] - regional_cap_ets2[i]) /
                      regional_cap_ets2[i] * 100) if regional_cap_ets2[i] > 0 else 0
            print(
                f"{region:<15} {regional_cap_ets2[i]:>12.1f} {regional_cap_2040[i]:>12.1f} {growth:>12.1f}")
    ...
    print("Error: 2030 data not found in the renewable capacity sheet.")
