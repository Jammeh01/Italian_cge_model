...
Updated to align with latest simulation structure
"""
...
# Extract 2021 and 2040 data
# Structure: Year (col 0), then BAU, ETS1, ETS2 for each sector/region
years_sectoral = sectoral_df.iloc[2:, 0].values
years_households = households_df.iloc[2:, 0].values

# Find indices for 2021 and 2040
idx_2021_s = np.where(years_sectoral == 2021)[0][0] + 2  # +2 for header rows
idx_2040_s = np.where(years_sectoral == 2040)[0][0] + 2

idx_2021_h = np.where(years_households == 2021)[0][0] + 2
idx_2040_h = np.where(years_households == 2040)[0][0] + 2

# Define emission sources matching model structure
sectors = ['Agriculture', 'Industry', 'Energy', 'Transport', 'Services']

# Extract sectoral emissions for 2021 (BAU - baseline year)
emissions_2021 = {}
col_offset = 1  # Start after Year column
for i, sector in enumerate(sectors):
    # Each sector has 3 columns: BAU, ETS1, ETS2
    emissions_2021[sector] = pd.to_numeric(
        sectoral_df.iloc[idx_2021_s, col_offset + i*3], errors='coerce')

# Extract household emissions for 2021 (sum of all regions)
household_2021 = 0
regions = ['Northwest', 'Northeast', 'Centre', 'South', 'Islands']
h_col_offset = 1
for i, region in enumerate(regions):
    val = pd.to_numeric(
        households_df.iloc[idx_2021_h, h_col_offset + i*3], errors='coerce')
    household_2021 += val if not pd.isna(val) else 0
emissions_2021['Households'] = household_2021

# Extract sectoral emissions for 2040 - all scenarios
emissions_2040_bau = {}
emissions_2040_ets1 = {}
emissions_2040_ets2 = {}
...
for i, sector in enumerate(sectors):
    ...
    emissions_2040_bau[sector] = pd.to_numeric(
        sectoral_df.iloc[idx_2040_s, base_col], errors='coerce')
    emissions_2040_ets1[sector] = pd.to_numeric(
        sectoral_df.iloc[idx_2040_s, base_col + 1], errors='coerce')
    emissions_2040_ets2[sector] = pd.to_numeric(
        sectoral_df.iloc[idx_2040_s, base_col + 2], errors='coerce')

# Extract household emissions for 2040 - all scenarios
household_2040_bau = 0
household_2040_ets1 = 0
household_2040_ets2 = 0

for i, region in enumerate(regions):
    base_col = h_col_offset + i*3
    val_bau = pd.to_numeric(
        households_df.iloc[idx_2040_h, base_col], errors='coerce')
    val_ets1 = pd.to_numeric(
        households_df.iloc[idx_2040_h, base_col + 1], errors='coerce')
    val_ets2 = pd.to_numeric(
        households_df.iloc[idx_2040_h, base_col + 2], errors='coerce')

    household_2040_bau += val_bau if not pd.isna(val_bau) else 0
    household_2040_ets1 += val_ets1 if not pd.isna(val_ets1) else 0
    household_2040_ets2 += val_ets2 if not pd.isna(val_ets2) else 0

emissions_2040_bau['Households'] = household_2040_bau
emissions_2040_ets1['Households'] = household_2040_ets1
emissions_2040_ets2['Households'] = household_2040_ets2

# Prepare data for plotting
sources = sectors + ['Households']
