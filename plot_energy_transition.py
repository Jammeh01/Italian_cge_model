...
# Electricity (column indices: BAU=7, ETS1=8, ETS2=9)
electricity_bau = pd.to_numeric(
    # Convert to TWh
    energy_df.iloc[2:, 7], errors='coerce').values[mask] / 1e9
electricity_ets1 = pd.to_numeric(
    energy_df.iloc[2:, 8], errors='coerce').values[mask] / 1e9
electricity_ets2 = pd.to_numeric(
    energy_df.iloc[2:, 9], errors='coerce').values[mask] / 1e9

# Gas (column indices: BAU=16, ETS1=17, ETS2=18)
gas_bau = pd.to_numeric(
    # Convert to TWh
    energy_df.iloc[2:, 16], errors='coerce').values[mask] / 1e9
gas_ets1 = pd.to_numeric(
    energy_df.iloc[2:, 17], errors='coerce').values[mask] / 1e9
gas_ets2 = pd.to_numeric(
    energy_df.iloc[2:, 18], errors='coerce').values[mask] / 1e9

# Other Energy (column indices: BAU=25, ETS1=26, ETS2=27)
other_bau = pd.to_numeric(
    # Convert to TWh
    energy_df.iloc[2:, 25], errors='coerce').values[mask] / 1e9
other_ets1 = pd.to_numeric(
    energy_df.iloc[2:, 26], errors='coerce').values[mask] / 1e9
other_ets2 = pd.to_numeric(
    energy_df.iloc[2:, 27], errors='coerce').values[mask] / 1e9

# Create figure with subplots for each scenario
fig, axes = plt.subplots(1, 3, figsize=(18, 5), sharey=True)

scenarios = ['BAU', 'ETS1 (Industry)', 'ETS2 (Building & Transport)']
electricity_data = [electricity_bau, electricity_ets1, electricity_ets2]
gas_data = [gas_bau, gas_ets1, gas_ets2]
other_data = [other_bau, other_ets1, other_ets2]

colors = {
    'Electricity': '#3498DB',  # Blue
    'Gas': '#E74C3C',          # Red
...
for idx, (ax, scenario) in enumerate(zip(axes, scenarios)):
    ...
    ax.fill_between(years, 0, electricity_data[idx],
                    color=colors['Electricity'], alpha=0.7, label='Electricity')
    ax.fill_between(years, electricity_data[idx],
                    electricity_data[idx] + gas_data[idx],
                    color=colors['Gas'], alpha=0.7, label='Gas')
    ax.fill_between(years, electricity_data[idx] + gas_data[idx],
                    electricity_data[idx] + gas_data[idx] + other_data[idx],
...
data_2021 = [
    electricity_bau[idx_2021],
    gas_bau[idx_2021],
    other_bau[idx_2021]
]
data_2040_bau = [
    electricity_bau[idx_2040],
    gas_bau[idx_2040],
    other_bau[idx_2040]
]
data_2040_ets1 = [
    electricity_ets1[idx_2040],
    gas_ets1[idx_2040],
    other_ets1[idx_2040]
]
data_2040_ets2 = [
    electricity_ets2[idx_2040],
    gas_ets2[idx_2040],
    other_ets2[idx_2040]
]
